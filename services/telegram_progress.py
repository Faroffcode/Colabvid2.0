...
class TelegramProgressReporter:
    ...
    async def start(self, *, file_name: str, total: int = 0) -> Any:
        ...
        self.message = await self.event.respond(format_pipeline_status(self.progress))
    ...
    async def update(self, payload: dict[str, Any], *, force: bool = False) -> None:
        ...
        async with self._lock:
            ...
            if not force and now - self._last_render < self.min_interval:
                return
            await self.message.edit(format_pipeline_status(self.progress))
    ...
    def _apply_payload(self, payload: dict[str, Any]) -> None:
        ...
        if stage == "downloading":
            ...
            if total:
                self.progress.download_percent = min(100, round(downloaded * 100 / int(total)))
                self.progress.download_size = f"{_size(downloaded)} / {_size(int(total))}"
            else:
            ...
            if clip is not None:
                self.progress.encoding_clip = f"{clip}/{total_clips or self.progress.total}"
            ...
            if status == "complete" and clip and total_clips:
                self.progress.encoding_percent = round(int(clip) * 100 / int(total_clips))
            ...
            if status == "complete":
                self.progress.completed = min(self.progress.total, self.progress.completed + 1)

        if payload.get("retries") is not None:
            self.progress.retries = int(payload["retries"])

    @staticmethod
    def callback(reporter: "TelegramProgressReporter", loop: asyncio.AbstractEventLoop):
        """Create a callback usable by synchronous pipeline workers."""
        def notify(payload: dict[str, Any]) -> None:
            asyncio.run_coroutine_threadsafe(reporter.update(payload), loop)
